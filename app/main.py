...
def getRadarChart(input_data):
    ...
    categories = ['radius','Texture','parimeter',
...
def show_predictions(input_data):
    model = pickle.load(open('Model/model.pkl', 'rb'))
    scaler = pickle.load(open('Model/scaler.pkl', 'rb'))

    input_array = np.array(list(input_data.values())).reshape(1,-1)
    scaled_input_array = scaler.transform(input_array)
    predictions = model.predict(scaled_input_array)
...
def main():
    ...
    with col1:
        getRadarChart(input_data)
        st.plotly_chart(getRadarChart(input_data))
    with col2:
        show_predictions(input_data)
